...
;;
;; このコピーは kotoba-lang/kami-genre-base-systems 用に schema-path のみ調整
;; （manifest/ が無いのでリポジトリルート直下 schema.edn）。他のロジックは
;; com-junkawasaki/root の manifest/edn-datomize.bb / kotoba-lang/security の
;; wrap-map-keep-ns 拡張版と同一。design/*.edn は既に :genre/*・:games/* の
;; 慣用的に名前空間付きキーのみで構成されているため wrap-map-keep-ns を使う
;; （このリポジトリでは games/*/deps.edn・scene.edn・scripts/verify 配下は
;; 生成物/実ツール設定として対象外 — 詳細はコミットメッセージ参照）。
